...
const createGig = async (req, res) => {
  try {
    const {
      title,
      userImage, // Assuming this is a buffer
      projectImages, // Assuming this is an array of buffers
      userName,
      userId,
      projectDetail,
    ...
    const newGig = new Gig({
      ...
      userId,
      projectDetail,
      skills, // Adding skills to the new gig
      contactInfo, // Adding contact information
      faq, // Adding FAQ to the new gig
    });
...
const getGigByProjectID = async (req, res) => {
  try {
      ...
      res.status(500).json({ error: 'An error occurred while retrieving the gig.' });
  }
};
...
const getAllGigsByFilter = async (req, res) => {
  try {
    ...
    const { page = 1, limit = 10, title, tags, skills, search, startDate, endDate, rating } = req.query;
...
const addReviewToGig = async (req, res) => {
    try {
        ...
        gig.rating = ((gig.rating * gig.reviews.length) + (newReview.rating || 0)) / (gig.reviews.length + 1);
